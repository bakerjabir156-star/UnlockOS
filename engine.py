...
_SIM_LOG_LINES = {
    ...
    "proxy_hijack": [
        ...
        ("INFO",    "PROXY",      "mitmproxy: Intercepting activation.apple.com..."),
...
def _detection_loop_real() -> None:
    ...
    while True:
        ...
        if device:
            ...
            dev_id = f"ios_{sn}"
            device["id"] = dev_id
            if dev_id not in processed_serials and dev_id not in _active_device_ids:
                processed_serials.add(dev_id)
        ...
        # Clear serial cache for devices that are no longer connected
        if not device and not adevice:
            processed_serials.clear()
