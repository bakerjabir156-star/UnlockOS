...
function openDFUModal() {
  $('dfu-modal-overlay').classList.add('show');
}

function closeDFUModal() {
  $('dfu-modal-overlay').classList.remove('show');
}

function showDFUTab(type) {
  // Tabs
  document.querySelectorAll('.dfu-tab').forEach(t => t.classList.remove('active'));
  const activeTab = document.querySelector(`.dfu-tab[onclick*="${type}"]`);
  if (activeTab) activeTab.classList.add('active');

  // Content
  $('dfu-old').style.display = type === 'old' ? '' : 'none';
  $('dfu-new').style.display = type === 'new' ? '' : 'none';
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  $('dfu-modal-overlay').addEventListener('click', (e) => {
    if (e.target === $('dfu-modal-overlay')) closeDFUModal();
  });
});
